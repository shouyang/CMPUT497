...
from nltk.tag import brill, UnigramTagger

UNK_TOKEN = "<UNK>"


def read_train_data(filepath, threshold = 1):
...
if __name__ == "__main__":
    ...
    train_data, word_counts = read_train_data(train_file, threshold)
    ...
    for sent in test_data:
        if sent:
            ...
            # Adjust the actual tagged sentence for unknown words
            adjusted_sent = []
            for word in sent:
                if threshold:
                    if word not in word_counts or word_counts[word] <= threshold:
                        word = UNK_TOKEN
                    adjusted_sent.append(word)

            raw_tagged_sent = model.tag(adjusted_sent)

            
            # Back out the actual words
            tagged_sent = []
            for idx, item in enumerate(raw_tagged_sent):
                new_tuple = (sent[idx], item[1])
                tagged_sent.append(new_tuple)
